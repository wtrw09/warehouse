...
Write-Host "=== AMD64 后端离线构建工具 ===" -ForegroundColor Green
Write-Host ""
...
switch ($choice) {
    "1" {
        Write-Host ""
        Write-Host "=== 直接构建 AMD64 后端镜像 ===" -ForegroundColor Cyan
        Write-Host ""
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host ""
            Write-Host "✓ AMD64 后端镜像构建成功！" -ForegroundColor Green
            Write-Host ""
    ...
    "3" {
        ...
        if (Test-Path "warehouse-backend-source.zip") {
            ...
            Write-Host "   scp warehouse-backend-source.zip user@server:/path/" -ForegroundColor White
            Write-Host ""
